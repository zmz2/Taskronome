...
| Microsoft.WindowsAppRuntime.Insights.Resource.dll | Windows App SDK 2.4.0 x64 self-contained runtime satellite；随应用目录分发 | 2.4.0.0 | MIT，Microsoft；与 `Microsoft.WindowsAppSDK` 2.4.0 的运行时匹配 |
...
`src/Taskronome.App/Runtime/Microsoft.WindowsAppRuntime.Insights.Resource.dll` 是 Microsoft Windows App SDK 2.4.0 x64 self-contained runtime 的官方 satellite DLL。它用于 Windows App SDK 的版本信息/诊断资源加载，不是 Taskronome 自有代码；其来源、版本和 SHA-256 记录在同目录的 provenance 文件中。
